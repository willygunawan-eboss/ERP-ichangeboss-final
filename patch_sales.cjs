...
if (!code.includes('isAddModalOpen')) {
  ...
  code = code.replace("const { data: salesOrders } = useSalesOrders();", "const { data: salesOrders } = useSalesOrders();\n  const [isAddModalOpen, setIsAddModalOpen] = useState(false);");
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar">
              ...
              >
                <div className="space-y-1.5">
                  ...
                  <input required type="text" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" name="customer" />
                </div>
